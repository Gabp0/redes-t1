...
int main(int argc, char const *argv[])
{
    ...
    // errors::genCRCtable((uint8_t)0xE7);
    // errors::genCRCtable((uint16_t)0xD175);
    ...
    uint8_t crc8slow = errors::crc8slow(data, size);
    printf("CRC 8 bits lento = 0x%02x\n", crc8slow);
    ...
    uint16_t crc16slow = errors::crc16slow(data, size);
    printf("CRC 16 bits lento = 0x%04x\n", crc16slow);
